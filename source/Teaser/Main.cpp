...
void createGrid()
{
	...
	LOCAL_PERSIST i8 tilesX = 20;
	LOCAL_PERSIST i8 tilesY = 20;
...
void main()
{
	...
	while (running)
	{
		...
		f32 time = SDL_GetTicks() / 2000.0;
		f32 r = 10;
		camPos = Vector3(sin(time), 0.5, cos(time))*r;
		g_camera = lookAt(camPos, Vector3(0, 0, 0), Vector3(0, 1, 0));
		...
		Matrix4 model = translate(0, 0.0, sin(SDL_GetTicks() / 500.0));
